...
function parsePillar(ch) {
  if (!ch || ch.length < 2) return { chinese: ch || '', stem:'', branch:'', stemElement:'', branchElement:'', animal:'' };
  const stem   = ch[0];
  const branch = ch[1];
  const stemEn   = HEAVENLY_STEMS_EN[stem]    || {};
  const branchEn = EARTHLY_BRANCHES_EN[branch] || {};
  return {
    chinese: ch, stem, branch,
...
export const handler = async (event) => {
  ...
  try {
    ...
    const d = DATES[year]?.[month]?.[day];
    if (!d) throw new Error(`No mapping for ${year}-${month}-${day}`);

    const earthKey   = getEarthKey(hour);
    const heavenKey  = HOUR_MAPPING[earthKey]?.[String(d.HDay)];

    const yearChinese  = TIANGAN['H' + d.HYear]  + DIZHI['E' + d.EYear];
    const monthChinese = TIANGAN['H' + d.HMonth] + DIZHI['E' + d.EMonth];
    const dayChinese   = TIANGAN['H' + d.HDay]   + DIZHI['E' + d.EDay];
    const timeChinese  = heavenKey ? TIANGAN[heavenKey] + DIZHI[earthKey] : '吉';
